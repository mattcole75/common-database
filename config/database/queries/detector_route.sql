select distinct
    ...
    h.detector,
    h.sequence as 'detector sequence',
    i.distance,
    i.type
from tms_route_section_link a 
    ...
    inner join tms_route_link_detector_sequence h on c.id = h.route_link_id
    inner join tms_detector i on h.detector = i.equipment_number
where 
    a.route_code = 1 
    and a.line_number = 2
order by a.sequence asc, h.sequence asc;